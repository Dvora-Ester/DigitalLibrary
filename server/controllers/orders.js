...
const ordersController = {
    ...
    try {
      ...
      for (const bookId of orderedBookIds) {
        ...
        if (!book) {
          return res
            .status(400)
            .json({ error: `Book with ID ${bookId} does not exist` });
        }
      }

      // בניית line_items לסטרייפ
      const line_items = [];
      for (const bookId of orderedBookIds) {
        const book = await booksModel.getById(bookId);
        line_items.push({
      ...
      const result = await ordersModel.add({
        userId,
        ccNumber,
        validity,
        cvv,
      ...
      const Bookmark_On_Page = 0;
      await library.add(userId, result.orderId, orderedBookIds, Bookmark_On_Page, res);

      res.status(201).json({ message: "Order added successfully", orderId: result.orderId });
    } catch (err) {
      console.error("Error adding the order:", err);
      res.status(500).json({ error: "Error adding the order" });
    }
  },
  ...
  //     const stripeSession = await stripe.checkout.sessions.retrieve(stripeSessionId);
  //     const amountTotal = stripeSession.amount_total / 100;
